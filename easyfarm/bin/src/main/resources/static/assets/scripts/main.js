	...
	Core.module = {
		...
		menu: function(){
			$(".js__menu_mobile").on("click",function(){
				if(!$("html.sidebar-activeA").length && $(window).width() > 800){
					$("html").toggleClass("sidebar-activeA");
					$("html").removeClass("sidebar-activeB");
				}
				else if(!$("html.sidebar-activeB").length && $(window).width() < 800){
					$("html").toggleClass("sidebar-activeB");
					$("html").removeClass("sidebar-activeA");
				}
				else if($("html.sidebar-activeA").length && $(window).width() > 800){
					$("html").removeClass("sidebar-activeA");
				}
			...
			$("body").on("click",function(event){
				if ($("html.sidebar-activeB").length && $(window).width() < 800
					|| $("html.sidebar-activeB").length && $(window).width() > 800){
					var selector = $(event.target);
